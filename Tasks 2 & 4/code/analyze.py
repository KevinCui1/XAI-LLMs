...
import pandas as pd
import torch
# Pin torch intra-op threads to the container CPU limit. os.cpu_count() reports
# the host's total cores (256), not the 8-CPU cgroup quota, so torch would
# otherwise run every matmul across 128 threads on 8 CPUs and thrash the
# scheduler (the extract_features forward pass crawled at ~15 s/batch). The
# OMP/MKL/OPENBLAS env vars (set before this import) cover BLAS/OpenMP; this
# covers torch itself.
...
def _init_worker():
    # Each forked search worker should use a single intra-op thread so the pool
    # (PARALLEL workers) stays within the 8-CPU quota: PARALLEL * 1 ~= 8, not
    # PARALLEL * 8. Prevents the parallel phase from re-introducing thread
    # over-subscription on top of the multiprocessing fan-out.
...
def search_feats(acts, states, feats, weights, dataset):
    ...
    if settings.PARALLEL < 1:
        # FakePool runs in the main process (already capped at 8 threads).
        pool = util.FakePool(settings.PARALLEL)
    else:
        # Real workers each pin themselves to 1 intra-op thread (see _init_worker).
        pool = mp.Pool(settings.PARALLEL, initializer=_init_worker)
